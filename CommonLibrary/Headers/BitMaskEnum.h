...
ReturnType<Size> operator|(const Type& lh, const Type& rh)
{
    return static_cast<ReturnType<Size>>(lh) | static_cast<ReturnType<Size>>(rh);
}
template <class Type, int Size = sizeof(Type), class Enable = std::enable_if_t<std::is_enum_v<Type>>>
ReturnType<Size> operator|=(const Type& lh, const Type& rh)
{
...
template <class Type, int Size = sizeof(Type), class Enable = std::enable_if_t<std::is_enum_v<Type>>>
ReturnType<Size> operator~(const Type& lh)
{
    return ~static_cast<ReturnType<Size>>(lh);
}
